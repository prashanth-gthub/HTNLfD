...
public class Graph {
   ...
   public void addGraph (TaskClass task1, DecompositionClass dec1,
   ...
   void merge(List<Node> x, List<Node> y, TaskModel taskModel){
      ...
      while (i < M && j < N) {
         if ( x.get(i).isEquivalent(y.get(j), taskModel) ) {
            ...
            if ( i - 1 >= 0 ) {
               x.get(i).parents.add(x.get(i - 1));
               x.get(i - 1).childs.add(x.get(i));
            } else
               roots.add(x.get(i));
            ...
            if ( opt[i + 1][j] >= opt[i][j + 1] ) {
               ...
               if ( j - 1 >= 0 ) {
                  y.get(j).parents.add(y.get(j - 1));
                  y.get(j - 1).childs.add(y.get(j));
               } else
                  roots.add(y.get(j));
