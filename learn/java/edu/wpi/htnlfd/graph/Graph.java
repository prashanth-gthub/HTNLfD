...
public class Graph {
	...
	/**
	 * Checks whether the current demonstration is satisfied by the model or
	 * not.
	 */
		...
		for (List<Node> nodes : nodesList) {
			...
			if (nodes.size() == demonstration.size()) {
				...
				for (int i = 0; i < nodes.size(); i++) {
					...
					if (!nodes.get(i).isEquivalent(demonstration.get(i),
							taskModel)) {
						equal = false;
						break;
					}
	...
	boolean mergable(Node root, List<Node> demonstration, TaskModel taskModel,
		...
		for (List<Node> nodes : nodesList) {
			...
			int LCS = getLCS(nodes, demonstration, taskModel, newNodes1,
					newNodes2);
	...
	private boolean merge(Node root, List<Node> demonstration,
		...
		while (secondD <= indicesDem.length - 1
			...
			if ((firstM == -1 && secondM == indicesMod.length)
				...
				TaskClass originalTaskCopy = new TaskClass(taskModel,
						originalTask);
				...
				if (!statisfiable(nodesListPermutation, nodeSteps, taskModel)) {
					...
					TaskClass newTask2Original = originalTask.addInternalTask(
							taskModel, steps.get(0).getDecompositionClass(),
							steps);
					...
					TaskClass newTask = steps
							...
							.addInternalTask(taskModel,
									steps.get(0).getDecompositionClass(),
									stepsD);
	...
	void permutation(List<ArrayList<Node>> nodesLists, List<Node> dem,
		...
		for (int i = 0; i < dem.size(); i++) {
			...
			if (!contain) {
				...
				for (Node node : nodes) {
					if (node.step != null) {
						Node temp = new Node(node.value, node.parent,
								node.children, node.typeOfChildren,
								node.typeOfNode, node.step, node.stepName,
								node.id, node.required);
						newNodes.add(temp);
					} else {
				...
				Node temp = new Node(dem.get(i).value, dem.get(i).parent,
						...
						dem.get(i).typeOfNode, dem.get(i).step,
						dem.get(i).stepName, dem.get(i).id, dem.get(i).required);
				newNodes.add(temp);
