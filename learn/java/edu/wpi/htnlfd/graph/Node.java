...
public class Node {
	...
	public Node(Node value, Node parent, List<Node> children,
			CType typeOfChildren, NType typeOfNode, int id,
	...
	public List<ArrayList<Node>> constantEval(Node root) {
		...
		if (root.children == null || root.children.size() == 0) {
			...
			if (root.step != null
					&& root.step.getDecompositionClass().isOptionalStep(
							root.stepName)) {
	...
	public boolean isEquivalent(Node comp, TaskModel taskModel) {
		...
		Step stepRealD = stepFakeD.getDecompositionClass().getStep(
				comp.stepName);
	...
	public Node addNode(Node node, CType required, TaskModel taskModel) throws ClassNotFoundException {
		...
		// Adding to taskClass
		Step which = (node.parent.children.get(node.parent.children
				.indexOf(node))).step;
