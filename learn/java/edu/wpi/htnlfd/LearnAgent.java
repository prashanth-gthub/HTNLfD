...
import edu.wpi.htnlfd.world.CarWorld;
...
public class LearnAgent extends Agent {
	...
	public Plugin.Item respondIf(Interaction interaction, boolean guess) {
		if (!once) {
			// logic goes in if above to decide when to ask question based on
			// state
			// of learned
			// model and Disco
			once = false;
			...
			if (question instanceof AskAppCondition) {
				return Agenda.newItem(Propose.Should.newInstance(disco, true,
			 ...
			 * Propose.Should.newInstance(disco, true,
			 * disco.getTaskClass("TellPostcondition").newInstance()), null);
			 * case "TellPrecondition": return Agenda.newItem(
			 * Propose.Should.newInstance(disco, true,
			 * disco.getTaskClass("TellPrecondition").newInstance()), null);
			 * case "TellOrdered": return Agenda.newItem(
			 * Propose.Should.newInstance(disco, true,
