...
public class TaskClass extends TaskModel.Member {
	...
	/**
	 * Sets the internal. It means that this is an intermediate goal and the
	 * name doesn't have any meaning to the user.
	 */
	 ...
	 * the same as the input of demonstrated task, it will return the name of
	 * the existed input. If it can't find it, it will add the input to the
	 * TaskClass.
	 * 
	 * @param taskModel
	 */
	...
	public abstract class Slot {
		...
		protected Element toNode(Document document, String slot) {
			Element element = document.createElementNS(TaskModel.xmlnsValue,
					slot);
			if (name == null || type == null)

				throw new IllegalArgumentException(
						"Name or type of slot is null: " + this);
		...
		public Element toNode(Document document, Properties properties) {
			...
			if (modified != null)
				TaskModel.addAttribute(document, element, "modified",
						modified.getName());
			if (optional)
				properties.put(getId() + "." + this.getName() + "@optional",
						"true");
			return element;
		}
	...
	public boolean isEquivalent(TaskClass next, TaskModel taskModel) {
		...
		if (this.getDeclaredInputs().size() == fakeTask.getDeclaredInputs()
			...
			for (Input thisIn : this.getDeclaredInputs()) {
				boolean contain = false;
				Iterator<Input> iterate = fakeTask.getDeclaredInputs()
						.iterator();
		...
		if (this.getDeclaredOutputs().size() == next.getDeclaredOutputs()
				.size()) {
			for (Output thisOut : this.getDeclaredOutputs()) {
				boolean contain = false;
				Iterator<Output> iterate = fakeTask.getDeclaredOutputs()
						.iterator();
				while (iterate.hasNext()) {
	...
	public List<String> addInputsBindings(TaskModel taskModel,
		...
		for (String inputName : step.getType().getDeclaredInputNames()) {
			...
			String changedName = this.addInput(taskModel, this, inputName, step
					.getType().getSlotType(inputName), step.getType()
			...
			if (inputNum1 != inputNum2) {
				...
				for (int i = this.getDeclaredOutputs().size() - 1; i >= (this
					...
					if (step.getType().getModified(inputName) != null
							&& this.getDeclaredOutputs()
									.get(i)
									.getName()
									.contains(
											step.getType().getModified(
													inputName))) {
	...
	public TaskClass addInternalTask(TaskModel taskModel,
		...
		DecompositionClass.Step taskStp = taskDecomposition.new Step(taskI, 1,
				1, null);
		...
		for (int i = 0; i < taskDecomposition.getStepNames().size(); i++) {
			if (taskDecomposition.getStep(
					taskDecomposition.getStepNames().get(i)).equals(
					steps.get(0))) {
				if (i - 1 >= 0)
						...
						if (in.getName().equals(
							...
							if ((inputT = taskI.getInput(in.getChangedName(
								...
								inputT = taskI.new Input(null, in.getType(),
										in.getModified());
								inputT.setName(inputT.getChangedName(
										in.getName(), stepName, stepNameNew));
							...
							subtask.addBinding("$" + stepNameNew + "."
									...
									subtask.new Binding(bind.getValue()
											.getSlot(), stepNameNew, "$this."
											+ inputT.getName(), bind.getValue()
											.getType()));

							addBindings
									.put("$" + stepNameTask + "."
											+ inputT.getName(),
											dec.new Binding(inputT.getName(),
													stepNameTask, bind
															.getValue()
															.getValue(), bind
															.getValue()
															.getType()));
							// add Constant
							Entry<String, Binding> bindR = dec.getBinding(bind,
									dec);
							if (bindR != null) {
								boolean contain = false;
				...
				if (bind.getValue().getValue().contains("$" + stepName + ".")) {
					...
					for (Output out : dec.getGoal().getDeclaredOutputs()) {
						if (out.getName().equals(bind.getValue().getSlot())) {
							...
							outputT.setName(outputT.getChangedName(
									out.getName(), stepName, stepNameNew));
			...
			if (taskDecomposition.getStep(rm) != null) {
				for (Entry<String, Binding> rmB : removeBindings) {
					if (rmB.getValue().getStep().equals(rm)
							|| rmB.getValue().getValue()
									.contains("$" + rm + ".")) {
						taskDecomposition.removeBinding(rmB.getKey());
						for (Entry<String, Binding> add : addBindings
								.entrySet()) {
