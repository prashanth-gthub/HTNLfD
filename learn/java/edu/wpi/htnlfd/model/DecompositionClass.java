...
public class DecompositionClass extends TaskModel.Member {
	...
	public String addStep(Step which, String stepName, Step step,
		...
		for (Entry<String, Binding> bind : prevDec.getBindings().entrySet()) {
			if (bind.getValue().getStep().equals(stepName)) {
				...
				String modified = (inputT.getModified() == null) ? null
						: inputT.getModified().getName();
				...
				if (!add) {
					currentDec.addBinding("$" + newName + "."
							...
							currentDec.new Binding(bind.getValue().getSlot(),
									newName, "$this." + inName,
					...
					if (modified != null) {
						...
						String outputTaskName = prevDec.getBindings()
								...
								.substring(2 + stepName.length());
						TaskClass.Output output = currentTask.new Output(
								currentTask.findOutputName(newName,
										outputTaskName), prevTask.getOutput(
										outName).getType());
						currentTask.addOutput(output);
						currentTask.getInput(inName).setModified(output);

						currentDec.addBinding("$this." + output.getName(),
								currentDec.new Binding(output.getName(),
										"this", "$" + newName + "."
												+ outputTaskName,
										DecompositionClass.Type.OutputOutput));
					}
	...
	public Node toNode(Document document, Set<String> namespaces) {
		...
		for (Entry<String, DecompositionClass.Binding> bind : this
				.getBindings().entrySet()) {

			subtasks.appendChild(bind.getValue()
					.toNode(document, bind.getKey()));
		}
		return subtasks;
	}
	...
	public class Step {
		...
		public boolean isEquivalentSteps(TaskModel taskModel,
				List<String> steps1, DecompositionClass dec1,
		...
		/**
		 * Finds goal, if the task is primitive it should be created otherwise
		 * it will be searched in the existed tasks.
		 */
				...
				for (String in : step.getType().getDeclaredInputNames()) {
					...
					for (TaskClass.Output out : goal.getDeclaredOutputs()) {
						if (step.getType().getModified(in) != null
								&& out.getName().equals(
										step.getType().getModified(in))) {
							inputC = goal.new Input(in, step.getType()
									.getSlotType(in), out);
							break;
						}
					}
					if (inputC == null) {
						inputC = goal.new Input(in, step.getType().getSlotType(
								in), null);
					}
		...
		for (Input in1 : type1.getDeclaredInputs()) {
			...
			for (Input in2 : type2.getDeclaredInputs()) {
				...
				if (!((in1.getModified() != null) ^ in2.getModified() != null)
					...
					String value1 = findValueInParents(taskModel, stp1, type1,
							dec1, in1.getName());
					String value2 = findValueInParents(taskModel, stp2, type2,
							dec2, in2.getName());
				...
				if (!((in1.getModified() != null) ^ in2.getModified() != null)
					...
					for (Entry<String, Binding> binding : dec1.getBindings()
						...
						if (binding.getValue().getStep().equals(stp1.getKey())
								&& binding.getValue().getSlot()
										.equals(in1.getName())) {
							value1 = getConstantValue(binding, stp1.getValue()
									.getDecompositionClass());
							if (value1 == null) {
								binding = getConstantBinding(binding, stp1
										.getValue().getDecompositionClass());

								int index = binding.getValue().getValue()
										.indexOf('.');
					...
					for (Entry<String, Binding> binding : dec2.getBindings()
						...
						if (binding.getValue().getStep().equals(stp2.getKey())
							...
							value2 = getConstantValue(binding, stp2.getValue()
									.getDecompositionClass());
							if (value2 == null) {
								binding = getConstantBinding(binding, stp2
										.getValue().getDecompositionClass());
								int index = binding.getValue().getValue()
										.indexOf('.');
	...
	public String findValueInParents(TaskModel taskModel, String stp,
		...
		List<Object[]> temp = findParents(parentTask, parentStep,
				parentSubtask, inputName, taskModel);
		if (temp.size() > 0) {
			...
			for (Entry<String, Binding> binding : dec.getBindings().entrySet()) {
				if (binding.getKey().equals(bindingRef.getKey())) {
					...
					if (binding.getValue().getType() == Type.Constant) {
						...
						contain = true;
						Entry<String, Binding> bindingR = getBinding(binding,
								dec);
						if (bindingR != null)
							bindingRef = bindingR;
						else
						...
						String outputName = binding.getValue().value
								.substring(binding.getValue().value
										.indexOf('.') + 1);
						...
						for (Entry<String, Binding> binding2 : dec
								.getBindings().entrySet()) {
	...
	/**
	 * Adds the ordering constraints according to the flow of inputs and
	 * outputs.
	 */
				...
				for (Entry<String, Binding> bindingRef : this.getBindings()
					...
					if (!bindingRef.getKey().contains("this") // &&
						...
						if (valueDep != null && valueRef != null
							...
							inputRef = bindingRef.getValue().getValue()
									.substring(6);

							for (TaskClass.Input inputs : task
									.getDeclaredInputs()) {
								if (inputs.getName().equals(inputRef)
										&& inputs.getModified() != null) {
									for (Entry<String, Step> step : this
											.getSteps().entrySet()) {
										if (step.getKey()
												.equals(bindingDep.getValue()
														.getStep())) {
											step.getValue().addRequired(
													bindingRef.getValue()
															.getStep());
											this.setOrdered(false);
											String stepOutputName = bindingRef
													.getKey()
													.substring(
															1,
															bindingRef
			...
			for (int i = 0; i < temp1.size(); i++) {
				...
				for (int j = 0; j < temp2.size(); j++) {
					...
					if (step1.isEquivalent(step2, taskModel)) {
						...
						if (checkInputs(temp1.get(i), step1.getType(),
								temp2.get(j), step2.getType(), this, dec,
								taskModel)) {
		...
		for (Entry<String, Step> step : this.getSteps().entrySet()) {
			...
			if (!step.getValue().getType().isInternal()) {
				...
				for (Input in : step.getValue().getType().getDeclaredInputs()) {
					String var = findValueInParents(taskModel, step.getKey(),
							step.getValue().getDecompositionClass().getGoal(),
							step.getValue().getDecompositionClass(),
							in.getName());
					if (var != null)
						val += var + " and";
				}
	...
	public void addOrderingByDataflow() {
		for (Entry<String, Binding> bind : this.bindings.entrySet()) {
			...
			if (!stepIn.equals("this")
					...
					&& this.getStep(stepIn).getType()
							.isInput(bind.getValue().slot)) {
				int st = bind.getValue().getValue().indexOf('.');
				if (st == -1)
					continue;
				String outputName = bind.getValue().getValue()
						.substring(st + 1);
