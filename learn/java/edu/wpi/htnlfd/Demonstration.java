...
public class Demonstration {
	 ...
	 * last demonstrations. Algorithm: Since we know that all of our
	 * demonstrated tasks are in "Demonstration" segment, we can just search for
	 * all of segments in that segment.
	 */
	...
	public TaskModel buildTaskModel(Disco disco, String taskName,
		...
		demonstrations.add(new Pair(taskName, newTask)); // saving
															// demonstrations
		...
		if (task != null) {
			 ...
			 * input; if ( task.getDecompositions().get(0).getApplicable() ==
			 * null ) { task.getDecompositions().get(0).setApplicable("this." +
			 * input); } TaskClass.Input inputC = task.new Input(input,
			 * "boolean", null); task.addInput(inputC);
	...
	public boolean addAlternativeRecipe(TaskClass newTask, String applicable,
		...
		if (task != null) {
			...
			newTask.getDecompositions()
					.get(0)
					.setId(task.getDecompositions().get(0)
			...
			for (TaskClass.Input in1 : task.getDeclaredInputs()) {
				...
				for (TaskClass.Input in2 : newTask.getDeclaredInputs()) {
					if (in1.getType().equals(in2.getType())
						...
						if (value == null) {
							for (int tg = 0; tg < task.getDecompositions()
									.size() - 1; tg++) {
								value = newTask
										.getDecompositions()
										.get(0)
			...
			for (Entry<String, Binding> rem : removed1.entrySet()) {
				...
				for (Entry<String, Binding> binding : newTask
					...
					if (binding.getValue().getValue().equals(rem.getKey())) {
						if (bindedInput.get(rem.getKey()) != null)
							binding.getValue().setValue(
									bindedInput.get(rem.getKey()));
					}
					String valOut = bindedOutput.get(binding.getKey());
					if (valOut != null) {
						bindedOutputADD.put(valOut, binding.getValue());
						bindedOutputRemove.put(binding.getKey(),
								binding.getValue());
					}
				}
				...
				for (TaskClass.Input in : newTask.getDeclaredInputs()) {
					if (in.getName().equals(rem.getKey().substring(6))) {
						...
						if (in.getModified() != null)
							newTask.getDeclaredOutputs().remove(
									in.getModified());
						break;
					}
				}
			...
			for (TaskClass.Output out : newTask.getDeclaredOutputs()) {
				...
				TaskClass.Output outCC = task.new Output(outputName,
						out.getType());
			...
			for (TaskClass.Input in : newTask.getDeclaredInputs()) {
				String inputName = newTask.getDecompositions().get(0).getId()
						+ "_" + in.getName();//
				...
				TaskClass.Input inputCC = task.new Input(inputName,
						in.getType(), outputModified);
				inputCC.setOptional(true);
				task.addInput(inputCC);
				String binding = newTask.getDecompositions().get(0)
						.getBindings().get("$this." + in.getName()).getValue();

				// input
				boolean binded = addSlotToParents(task, newTask
						.getDecompositions().get(0), taskModel, true,
						inputName, in.getType(), binding, modified);
	...
	/**
	 * Adds the slot to parents. If a subtask is added to one of the child
	 * tasks, this function adds it's input or output to it's parents
	 */
	public boolean addSlotToParents(TaskClass parentTask,
			DecompositionClass parentSubtask, TaskModel taskModel,
			boolean type, String slotName, String slotType, String binding,
			String modified) {
		List<Object[]> parents = parentSubtask.findParents(parentTask, null,
				parentSubtask, null, taskModel);

		String lastInputName = this.inputTransformation.transferBottomUp(
				parents, slotName, slotType, modified, type);

		if (type && parents != null && parents.size() != 0) {

			DecompositionClass subtask = (DecompositionClass) parents
					.get(parents.size() - 1)[1];

			// @SuppressWarnings("unchecked")
			// Entry<String, Step> step = (Entry<String, Step>)
			// parents.get(parents
			// .size() - 1)[2];

			subtask.addBinding("$this." + lastInputName, subtask.new Binding(
					lastInputName, "this", binding,
	...
	public void learnedTaskmodel() {
		...
		while (tasksIterator.hasNext()) {
			...
			TaskClass domTask = new TaskClass(taskModel, task.getId(),
					new QName(task.getQName().getNamespaceURI(), task
			...
			for (String inputName : task.getDeclaredInputNames()) {
				...
				String optional = task.getProperty(task.getId() + "."
						+ inputName + "@optional");
			...
			for (edu.wpi.cetask.DecompositionClass subtaskDecomposition : decompositions) {
				...
				for (String stepName : subtaskDecomposition.getStepNames()) {
					...
					TaskClass taskType = this.taskModel
							.getTaskClass(subtaskDecomposition.getStepType(
									stepName).getId());
					if (taskType == null) {
						taskType = new TaskClass(taskModel,
								subtaskDecomposition.getStepType(stepName)
										.getId(), subtaskDecomposition
										.getStepType(stepName).getQName());

						for (String out : subtaskDecomposition.getStepType(
								stepName).getDeclaredOutputNames()) {

							TaskClass.Output outputTask = taskType.new Output(
									out, subtaskDecomposition.getStepType(
											stepName).getSlotType(out));
							taskType.addOutput(outputTask);

						}

						for (String in : subtaskDecomposition.getStepType(
								stepName).getDeclaredInputNames()) {

							TaskClass.Input inputC = null;
							for (TaskClass.Output out : taskType
									.getDeclaredOutputs()) {
								if (subtaskDecomposition.getStepType(stepName)
										.getModified(in) != null
										&& out.getName().equals(
												subtaskDecomposition
														.getStepType(stepName)
														.getModified(in))) {
									inputC = taskType.new Input(in,
											subtaskDecomposition.getStepType(
													stepName).getSlotType(in),
											out);
					...
					DecompositionClass.Step step = subtask.new Step(taskType,
							1, 1, null);
					// since isOrdered is true so this function
					// getRequiredStepNames(stepName) will return all of the
					// required
					// steps
				...
				for (Entry<String, edu.wpi.cetask.DecompositionClass.Binding> binding : bindingsSubtask) {
					...
					subtask.addBinding(binding.getKey(), subtask.new Binding(
							binding.getValue().slot, binding.getValue().step,
							binding.getValue().value, type)); // Error
			...
			for (DecompositionClass dec : mytask.getDecompositions()) {
				for (String stepName : dec.getStepNames()) {
					...
					if (step.getType().getDecompositions() == null
						...
						TaskClass taskCl = taskModel.getTaskClass(step
								.getType().getId());
						if (taskCl != null)
							step.setType(taskCl);
					}
	...
	public TaskModel addMaxOccurs(String subtaskId, String stepName,
		...
		for (int j = i + 1; j < subtask.getStepNames().size()
			...
			DecompositionClass.Step stepF = subtask.getStep(subtask
					.getStepNames().get(j));
	...
	public TaskModel answerQuestion(String taskName, String input) {
		...
		for (DecompositionClass dec : task.getDecompositions()) {
			dec.setApplicable(dec.getApplicable().replace(defaultInputName,
					input));
		}
	...
	/**
	 * Finds whether there is a loop in the steps of the task or not by
	 * searching for more than one consecutive same step.
	 */
			...
			for (Entry<Step, List<Step>> step : steps.entrySet()) {
				...
				for (int i = 0; i < list.size() - 1; i++) {
					...
					for (Output out : removeStep.getType().getDeclaredOutputs()) {
						Entry<String, Binding> binding = dec
								.removeBindingValue("$" + stepName + "."
										+ out.getName());
				...
				for (int j = 0; j < temp2.size(); j++) {
					...
					if (step1.isEquivalent(step2, taskModel)) {
						...
						if (subtask.checkInputs(temp1.get(i), step1.getType(),
								temp2.get(j), step2.getType(), subtask,
								supertask, taskModel)) {
			...
			while (iterator2.hasNext()) {
				...
				if (!next1.equals(next2)) {
					for (DecompositionClass dec1 : next1.getDecompositions()) {
						for (DecompositionClass dec2 : next2
								.getDecompositions()) {
