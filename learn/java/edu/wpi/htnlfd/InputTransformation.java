...
public class InputTransformation {
	 ...
	 * 
	 * @param parents
	 *            the parents (TaskClass, DecompositionClass, Step)
	 * @param type
	 ...
	 * 
	 * @param children
	 *            the children(TaskClass, DecompositionClass, Step)
	 * @param type
	 *            the type: Input(+modified output) or Output
	 */
	void transferTopDown(List<Object[]> children, String slotName, boolean type) {
		for (Object[] child : children) {
			...
			if (type) {
				Entry<String, Binding> key = subtask
						.removeBindingValue("$this." + slotName);
	...
	public void generalizeInput(TaskModel taskModel) {
		Iterator<TaskClass> tasksIterator = taskModel.getTaskClasses()
				.iterator();
		TaskClass task = null;
		...
		while (tasksIterator.hasNext()) {
			...
			for (DecompositionClass subtask : task.getDecompositions()) {
				...
				for (Entry<String, Step> step : subtask.getSteps().entrySet()) {
					...
					if (taskTemp != null) {
						for (Input input : taskTemp.getDeclaredInputs()) {
							...
							if (subtask.getBindingStep(step.getKey(),
									input.getName()) == null) {
				...
				for (int i = 0; i < stepsInputs.size(); i++) {
					...
					// If all of the steps have this input, add the binding to
					// them
					if (num == subtask.getSteps().size() - 1) {
						...
						for (Entry<String, Step> step : subtask.getSteps()
							...
							subtask.addBinding("$" + step.getKey() + "."
									...
									subtask.new Binding(stepsInputs.get(i)
											.getName(), step.getKey(), "this."
											+ stepsInputs.get(i).getName(),
			...
			for (int k = 0; k < decompositionInputs.size(); k++) {
				...
				if (!task.hasInput(decompositionInputs.get(k).getName())) {
					task.addInput(task.new Input(decompositionInputs.get(k)
							.getName(), decompositionInputs.get(k).getType(),
							null));
				}
			}
