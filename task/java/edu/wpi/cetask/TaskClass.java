...
public class TaskClass extends TaskModel.Member {
   ...
   TaskClass (Node node, TaskModel model, XPath xpath) { 
      ...
      if ( declaredInputNames.contains("external") )
         throw new ReservedSlotException("external");
      // efficiency not an issue for tiny lists here
      for (String name : declaredInputNames)
         if ( declaredInputNames.indexOf(name) 
               != declaredInputNames.lastIndexOf(name) )
      ...
      if ( declaredOutputNames.contains("success") )
         throw new ReservedSlotException("success");
      if ( declaredOutputNames.contains("external") )
            throw new ReservedSlotException("external");
      for (String name : declaredOutputNames)
         if ( declaredOutputNames.indexOf(name) 
               != declaredOutputNames.lastIndexOf(name) )
            throw new DuplicateSlotNameException(name);
      outputNames =  new ArrayList<String>(declaredOutputNames);
      outputNames.add("success"); outputNames.add("when");
      // cache slot types
      slotTypes = new HashMap<String,String>(inputNames.size()+outputNames.size());
      for (String name : inputNames) slotTypes.put(name, slotType(name));
      for (String name : outputNames) slotTypes.put(name, slotType(name));
   ...
   private String slotType (String name) {
      if ( name.equals("success") || name.equals("external") )
         return "boolean";
      else if ( name.equals("when") ) return "Date";
   ...
   private final Map<String,String> slotTypes;
    ...
    *          defined in the current ECMAScript environment
    */
   public String getSlotType (String name) { return slotTypes.get(name); }
    
   /**
    * Tests whether given string is name of a slot of this task class.
    */
   public boolean isSlot (String name) {
      return inputNames.contains(name) || outputNames.contains(name);
   }
