...
public class Shell {
   ...
   protected Shell (TaskEngine engine, String from) {
      ...
      log = new File(System.getProperty("java.io.tmpdir") + '/'
            + (source == null ? 
   ...
   protected void init (TaskEngine engine) {
      ...
      try {
         ...
         if ( source == null ) {
            ...
            setErr(new PrintStream(new CopyOutputStream(err, logStream), true));
         }
   ...
   private void copyOutput (PrintStream logStream) {
      setOut(new PrintStream(new CopyOutputStream(out, logStream), true));
      setErr(new PrintStream(new CopyOutputStream(err, logStream), true));
   }
   ...
   public void exception (Throwable e) {
      if ( TaskEngine.DEBUG ) throw new RuntimeException(e);
      else {
         err.println(e); 
   ...
   private static boolean nextArg (StringTokenizer tokenizer, 
      ...
      if ( tokenizer.hasMoreTokens() ) {
         ...
         if ( next.length() > 0 )
            task.setSlotValueScript(name, next, "Guide");
         return true;
      } else return false;
   }
