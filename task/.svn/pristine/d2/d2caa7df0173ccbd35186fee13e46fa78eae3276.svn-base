 ...
 *     See edu.wpi.disco.Interaction class for multi-thread support.
 */
public class TaskEngine {
   ...
   public String getDeviceType (Task task) { return deviceType; }
   public String getPlatform (Task task) { return platform; }
    ...
    * tasks is outside the scope of the standard, and is not addressed by
    * this implementation.
    */
      ...
      if ( INVOCABLE && SCRIPTABLE ) 
         try {
            return scriptEngine.isScriptable(value) ?
               (String) ((Invocable) scriptEngine)
                   .invokeFunction("edu_wpi_cetask_toString", value) :
               value+"";
      ...
      if ( !valid ) {
         if ( from != null ) // re-validate to get line numbers in file
            try {
               ...
               handler.from = Utils.toURL(from).getFile(); 
   ...
   private void print (SAXException e) {
      getErr().println(handler.from+": "+e.getMessage()); 
   }
   ...
   public TaskClass getTaskClass (String id) {
      TaskClass found = null;
      for (TaskModel model : models.values()) {
         TaskClass task = model.getTaskClass(id);
         if ( task != null ) {
            if ( found == null ) found = task;
            else throw new AmbiguousIdException(id+" found in "
                  +task.getNamespace()+" and "+found.getNamespace());
         }
      }
      return found;
   }
   ...
   public Plan done (boolean external, Task occurrence, Plan contributes) { 
      ...
      occurrence.done(); // does setWhen
      occurrence.evalIf(contributes);
