...
public class DecompositionClass extends TaskModel.Member {  
   ...
   private final static Pattern pattern = // to match $step.slot
   ...
   DecompositionClass (Node node, TaskModel model, TaskClass goal, XPath xpath) { 
      ...
      for (String name : stepNames) {
         ...
         // not checking for cycles in requires
      ...
      try { 
         synchronized (bindings) {
            ...
            for (Binding binding : bindings.values()) {
               ...
               while ( matcher.find() ) {
                  Binding depend = bindings.get(matcher.group());
                  if ( depend == null ) {
      ...
      applicable = xpath("./n:applicable");
      compiledApplicable = TaskEngine.isCompilable() ?
         engine.compile(applicable, this+" applicable") : null;
   }
   ...
   void updateBindings (Decomposition decomp, String retractedStep, String retractedSlot) {
      for (Binding binding : bindings.values()) 
  ...
  class Binding {
      ...
      private final String variable, step, slot, value, expression,
                           identityStep, identitySlot, where;
      private final Compiled compiled;
      private final boolean identity, inIn;
      ...
      private void update (Decomposition decomp, int depth, 
         ...
         for (Binding depend : depends) {
            ...
            if ( !equals(depend) && !dependTask.isDefinedSlot(depend.slot) ) {
               ...
               if ( inIn && target.isDefinedSlot(slot) ) 
         ...
         // note undefined binding value ignored below
         if ( value != null ) {
            if ( identity) 
