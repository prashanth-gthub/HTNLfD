...
public class Disco extends TaskEngine {
   ...
   // TODO How to handle domain models spread out over several model files
   // TODO Make parser not static?
   // TODO Think about namespaces for tasks and operators
   // TODO What if pre/post are unknown
   
   private UngroundProblem problem = new UngroundProblem();
   private EcmaScript parser = new EcmaScript(System.in);
    ...
    * @see #load(String)
    */
   @SuppressWarnings("unchecked") // for parseFF below
   public TaskModel load (String from, Document document, 
      ...
      if ( model != null && model.getProperty("parseFF", false)) {
         for (TaskClass task : model.getTaskClasses()) {
            UngroundInstantAction action = new UngroundInstantAction();
            problem.actions.add(action);
            action.name = new OperatorName(task.getId());
            for (String name : task.getDeclaredInputNames()) addParameter(action, task, name);
            for (String name : task.getDeclaredOutputNames()) addParameter(action, task, name);
            action.condition = TrueCondition.getInstance();
            processCondition(task.precondition);
            action.effect = NullEffect.getInstance();
            processCondition(task.postcondition);
            action.PDDLPrint(getOut(), 0);
         }
      }
      return model;
   }
   ...
   private void addParameter (UngroundInstantAction action, TaskClass task, String name) {
      Variable v = new Variable(name);
      action.params.add(v);
      String type = task.getSlotType(name);
      SimpleType stype = (SimpleType) problem.typeMap.get(type);
      if ( stype == null ) {
         stype = new SimpleType(type);
         problem.types.add(stype);
         problem.typeMap.put(type, stype);
      }
      v.setType(stype);
   }
   
   private void processCondition (String condition) {
      parser.ReInit(new ByteArrayInputStream(condition.getBytes()));
      try { parser.Program(); }
   ...
   /**
    * Return the plan that would be focus if exhausted plans popped off stack.
    * 
    * @param ignoreAccept if true then substitute non-null parent of Accept
    */
   ...
   private void push (Plan plan, boolean continuation) {
      ...
      stack.push(new Segment(plan, getSegment(), continuation));
   ...
   public void putUtterance (Utterance utterance, String formatted) {
      if ( utterances.get(utterance) == null ) // in case copied or set in TTSay
   ...
   public Task copy (Task task) {
      ...
      if ( task instanceof Utterance ) { // do not check occurred!
         ...
         String translated = ((Disco) task.engine).utterances.get(task);
         if ( translated != null ) utterances.put((Utterance) thisTask, translated); 
      }
      return thisTask;
   }
   ...
   private boolean print (PrintStream stream, int indent, Segment parent,
      ...
      boolean printed = !isEmpty() || (history && stack.get(0).children().hasNext());
   ...
   public void print (Task task, PrintStream stream, int indent) {
      ...
      if ( TaskEngine.DEBUG ) {
         stream.print(task.isUser() ? "User: " : "Agent: ");
         stream.print(task);
   ...
   private void print (Plan plan, PrintStream stream, int indent) {
      ...
      plan.print(stream, indent, false, !TaskEngine.DEBUG,
            // pre-format utterances (re translations)
            !TaskEngine.DEBUG && goal instanceof Utterance ?
    ...
    *
    * @param endSentence controls whether to call {@link Utils#endSentence(StringBuffer)}
    */
   public String formatUtterance (Utterance utterance, boolean endSentence) {
      StringBuffer buffer = new StringBuffer(Utils.capitalize(
   ...
   private final Map<Utterance,String> utterances 
   ...
   public String format (Task task) {
      if ( task instanceof Utterance && task.occurred() ) {
         ...
         String translated = utterances.get(task);
   ...
   private String prependStack (String key, boolean popExhausted) {
      StringBuffer buffer = new StringBuffer();
      appendStackNoVerbose(buffer, getSegment(), popExhausted);
      return buffer.append(key).toString();
   }

   private void appendStackNoVerbose (StringBuffer buffer, Segment segment,
                                      boolean popExhausted) {
      boolean verbose = TaskEngine.VERBOSE;
      try {
         TaskEngine.VERBOSE = false;
         buffer.append("[]"); // for toplevel translations
         appendStack(buffer, segment, popExhausted);
         buffer.append('_');
      } finally { TaskEngine.VERBOSE = verbose; }
   }
   
   private void appendStack (StringBuffer buffer, Segment segment,
                             boolean popExhausted) {
      if ( segment != null && segment.getPurpose() != null ) {
         ...
         if ( !popExhausted || !segment.getPlan().isExhausted() )
            buffer.append('[').append(segment.getPurpose()).append(']');
      }
   }
